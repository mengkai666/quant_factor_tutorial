...
回溯基础: 2025-11-06 ~ 2026-07-22 共 173 个交易日的涨停/跌停/A-D 全量样本。
每个场景的仓位建议均标注了历史胜率 (T+1 破新高% / T+3 破新高% / T+3 腰斩%)。

核心因子 (按预测力排序):
  ① ad_ratio  = up / (up + down)       # 情绪水位 (最强单因子)
  ② h_drop    = prev_max_h - max_h     # 断板变化 (最强顶部信号)
  ③ ladder    = h3 + 2h4 + 3h5 + 4h6+  # 梯队完整度
  ④ 突破前压力位 (5日高) + 情绪+梯队 联合  # 单变量胜率仅 48%, 三因子后锐化到 100%
  ⑤ 涨停/跌停家数放缩

反直觉发现 (来自 173 天回测):
  · "突破压力位" 单变量胜率仅 48.6%, 基本就是掷硬币
  · 梯队"饱满"反倒有 38% 的 T+1 腰斩率 (常在情绪高潮末段)
  · ad_ratio ∈ [0.20, 0.35] 是散户绞肉机 (破新高率仅 34%)
  · 冰点抄底 (ad<0.20) 是 T+3 破新高 75%、零腰斩的最强信号
  · 退潮防守 T+3 反弹率高达 82.4%, 是买入而非卖出的窗口

v2.1 反身性顶部保护 (2026-07-26 实盘教训):
  7/23 尾盘 A/D=0.78 触发 E_主升加速 建议 7-9 成, 7/24 情绪崩塌 A/D=0.17. 复盘:
  · 历史 E 案例 A/D 均值仅 0.57, 0.75+ 属于极端过热, 反身顶特征
...
def _classify_scene(curr_h, prev_h, ad, ladder, zt, dt, pressure_5d,
    ...
    if ad is not None and ad < 0.20 and curr_h <= 4:
        return (
            ...
            f'历史 173 天同型 4 例, T+3 破新高 75%, 零腰斩. 此时不进攻更待何时.',
            0.75
        )

    # ============================================================
    # F · 顶部崩塌 (断板 ≥3板 + 跌停 >15, 11.1% 概率 3 日内见 ≤3板)
    # ============================================================
    if h_drop >= 3 and dt > 15:
        return (
            ...
            f'历史该组合 9 例, 3 日内见 <=3板 概率 11%, 亏钱效应扩散最快.',
            0.11
        )

    # ============================================================
    # A+ · 突破共振 (突破压力位 + 情绪强 + 梯队饱满, 历史 3/3 破新高)
    ...
    if is_breakout and curr_h >= 5:
        ...
        if ad is not None and ad > 0.65 and ladder is not None and ladder >= 12:
            return (
                ...
                f'历史 3 例 100% 破新高 (小样本, 谨慎). 主力龙头小仓突破加, 大仓底仓不动.',
                0.90
            )
        # A 普通突破 - 25% 陷阱, 减仓不加仓
        return (
            ...
            f'历史 8 例仅 25% T+3 破新高, 5 例 T+1 直接退潮. '
            f'突破日应减仓, 而非加仓. 等 D 冰点信号才是真买点.',
            0.25
        )

    # ============================================================
    # E · 主升加速 (胜率 T+3 71.4%, 但腰斩率 35.7% - 强而不稳)
    ...
    if ad is not None and ad > 0.65 and curr_h >= 6 and (ladder is None or ladder >= 8):
        ...
        if overheat or broken_ladder:
            reasons = []
            if overheat:
                reasons.append(f'情绪极端过热 A/D={ad:.2f} (>0.75, 历史 E 均值 0.57 的顶部区)')
            if broken_ladder:
            ...
            return (
                ...
                f'历史该形态 T+1 情绪崩塌概率 >50%, 尾盘建议减仓而非加仓. '
                f'等 D 冰点信号 (A/D<0.20) 才是真买点.',
                0.35
            )
        return (
            'E_主升加速', '6-7成',
            ...
            f'历史 14 例 T+3 破新高 71%, 但 36% 腰斩. 死拿龙一底仓,尾盘绝不加仓,盯 T+2 分歧兑现.',
            0.71
        )

    # ============================================================
    # C · 高位分歧 (max_h≥6 且情绪弱, T+3 破新高 50%)
    # ============================================================
    if curr_h >= 6 and ad is not None and ad < 0.40:
        return (
            ...
            f'历史 8 例 T+3 破新高 50%、腰斩 25%. 前排逢高兑现, 空仓等新主线.',
            0.50
        )

    # ============================================================
    # B · 退潮蓄势 (T+3 反弹率 82%, 是买入信号非卖出)
    # ============================================================
    if h_drop >= 1 and ad is not None and ad < 0.35 and zt < 80:
        return (
            ...
            f'但历史 53 例 T+3 反弹率高达 82%, 退潮反倒是埋伏窗口. '
            f'底仓不动, 盯紧下一轮冰点抄底.',
            0.82
        )

    # ============================================================
    # 默认 · 中性震荡 (T+3 反弹率 76%, 保底 5 成)
    # ============================================================
