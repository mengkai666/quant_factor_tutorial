...
def _height(row: dict[str, Any]) -> int:
    import re
    value = row.get("height", row.get("连板高度", row.get("连板数", 0)))
...
def build_daily_delta_snapshot(current: dict[str, Any] | None, previous: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if not previous:
        return {"available": False, "reason": "缺少上一交易日结构化快照", "metrics": {}, "highlights": []}
    ...
    return {"available": True, "reason": "", "metrics": metrics, "highlights": highlights[:5]}
